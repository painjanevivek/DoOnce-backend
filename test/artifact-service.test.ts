...
const leaseToken = "valid-lease-token-value-that-is-long-enough________";
...
test("creates a signed expiring grant and verifies bytes before download", async () => {
  const metadata = new MemoryMetadata(); const objects = new MemoryObjects(); const service = new ArtifactService(metadata, objects, "artifact-signing-secret-that-is-long-enough");
  const artifact = await service.create(user, runId, { fileName: "evidence.json", contentType: "application/json", retentionClass: "publication-evidence", stepId, base64: Buffer.from("{}").toString("base64"), leaseToken });
...
test("rejects malformed base64 and oversized artifacts", async () => {
  ...
  await assert.rejects(() => service.create(user, runId, { fileName: "bad.txt", contentType: "text/plain", retentionClass: "debug", base64: "***", leaseToken }), ArtifactInputError);
